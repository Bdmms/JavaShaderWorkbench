...
public class Workbench extends JFrame
{
	...
	public Workbench()
	{
		...
		uniforms.add( "diffuse", GLDataType.SAMP2D );
	...
	public JMenuBar createMenuBar()
	{
		...
		file.add( createMenuItem( "Open", e -> 
		{
			chooseFile( this, System.getProperty( "user.dir" ), false, openFile ->
			{
				try 
				{
					_modelTree.add( ModelUtils.readObjFileDirect( openFile ) );
					_view.recompile();
				} 
		...
		file.add( createMenuItem( "Save As...", e -> 
		{
			chooseFile( this, null, true, saveFile ->
			{
				try 
				{
					LeafNode node = _editor.getSelectedNode();
	...
	/**
	 * Workbench.chooseFile( null, null, false, imageFile ->
		{
			try 
			{
				new MatrixColor( imageFile ).normalMap().writeTo( Workbench.chooseFile( null, null, true ) );
			} 
			catch (IOException e1) { e1.printStackTrace(); }
		} );
	 */
